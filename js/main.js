...
// ==================== AUTOPLAY PARA AMBOS OS VÍDEOS (HERO E SOBRE) ====================
document.addEventListener('DOMContentLoaded', function() {
    const videos = document.querySelectorAll('.dashboard-video, .sobre-video');
    
    const playVideo = (video) => {
        if (video && video.paused && video.readyState >= 2) { // readyState >= 2 = dados suficientes
            video.play().catch(e => {
    ...
    // Tentar tocar todos os vídeos assim que possível
    videos.forEach(video => playVideo(video));

    // Estratégia para dispositivos móveis: no primeiro toque, tenta novamente
